...
const Drawing = (function() { // Drawing in Canvas
    class Drawing {
        ...
        draw(oQRCode) {
            ...
            const _bkgCanvas = new Canvas(size, size);
            ...
            if (_htOption.backgroundImage !== undefined) {
                ...
                if (_htOption.autoColor) {
                    ...
                    const avgRGB = getAverageRGB(_htOption.backgroundImage);
            ...
            const agnPatternCenter = QRUtil.getPatternPosition(oQRCode.typeNumber);
            ...
            for (let row = 0; row < nCount; row++) {
                for (let col = 0; col < nCount; col++) {
                    ...
                    const isBlkPos = ((col < 7 && (row < 7 || row >= nCount - 7)) || (col >= nCount - 7 && row < 7));
            ...
            if (false && _htOption.binarize) {
                const pixels = _oContext.getImageData(0, 0, size, size);
                let threshold = 128;
            ...
            const _fCanvas = new Canvas(rawSize, rawSize); // document.createElement("canvas");
